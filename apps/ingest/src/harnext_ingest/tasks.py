...
from harnext_shared import CloudEvent, make_engine, make_sessionmaker
...
@app.task(name="harnext_ingest.tasks.dispatch_due_polls")
...
@app.task(name="harnext_ingest.tasks.poll_source")
...
async def _poll_source(source_id: str) -> int:
    ...
    try:
        ...
        if src is not None and src.kind == "sitemap":
            ...
            from harnext_ingest.crawler import crawl_sitemap
